...
export const getDeviation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!validCoins.includes(coin as string)) {
      res.status(400).json({
        error: `Invalid coin parameter. Valid options are: ${validCoins.join(', ')}`,
      });
      return;
    }
    ...
    const prices = records
      .map((record: { price: number | undefined }) => record.price)
    ...
    const variance =
      prices.reduce((sum: number, price: number) => sum + Math.pow(price - mean, 2), 0) /
      prices.length;
